...
Такой подход позволяет распараллелить наиболее массовую часть обработки — проход по большому количеству точек траекторий.
...
В проекте используется синтетический набор данных. То есть строки CSV не являются!!! реальными измерениями воздушных объектов, а создаются программно с помощью скрипта:
...
Для запуска серии измерений на 1–12 MPI-процессах используется скрипт:
...
4. На MacBook M1 Pro лучший результат в данном эксперименте достигнут при 9 MPI-процессах.
5. При дальнейшем увеличении числа процессов эффективность снижается.
